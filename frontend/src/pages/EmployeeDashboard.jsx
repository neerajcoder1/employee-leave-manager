...
const DashboardSkeleton = () => (
  <div className="dashboard-container">
    ...
    <div className="grid-balances">
      <div className="card-panel skeleton-pulse" style={{ height: '140px' }}></div>
      <div className="card-panel skeleton-pulse" style={{ height: '140px' }}></div>
      <div className="card-panel skeleton-pulse" style={{ height: '140px' }}></div>
    </div>
...
const EmployeeDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      addToast('error', 'Sync Failure', 'Failed to retrieve profile or history.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDrop = (e) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (allowedExtensions.includes(fileExt)) {
        ...
        setFormError('Invalid file type. Please upload a PDF, Word document, or image.');
      }
    }
  };
  ...
  const handleApplyLeave = async (e) => {
    ...
    try {
      ...
      addToast('success', 'Application Submitted', 'Your leave request has been submitted.');
  ...
  const filteredLeaves = leaves
    .filter((l) => {
      const matchesSearch = l.leave_type.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'duration') {
        const durA = Math.round((new Date(a.end_date) - new Date(a.start_date)) / (1000 * 60 * 60 * 24)) + 1;
        const durB = Math.round((new Date(b.end_date) - new Date(b.start_date)) / (1000 * 60 * 60 * 24)) + 1;
        return durB - durA;
      }
      return 0;
    });
  ...
  const casualAvailable = profile?.balances?.casual ?? 0;
  const casualUsed = Math.max(10 - casualAvailable, 0);
  ...
  return (
    <div className="dashboard-container">
      ...
      <header className="navbar">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          ...
          <button 
            ...
            onClick={() => addToast('info', 'Notifications', 'All catches up! No new alerts.')}
          ...
          >
            <span className="avatar-initials" style={{ transition: 'transform 0.2s ease, box-shadow 0.2s ease' }}>{getInitials(profile?.username)}</span>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', lineHeight: 1.2 }}>
              <span style={{ fontSize: '0.85rem', fontWeight: '600', transition: 'color 0.2s ease' }}>{profile?.username}</span>
      ...
      <section className="card-panel blur-reveal" style={{ padding: '2.5rem 2rem', textAlign: 'center', marginBottom: '2.5rem', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '12px' }}>
        ...
        <p style={{ fontSize: '0.95rem', color: 'var(--text-secondary)', marginBottom: '1.75rem', maxWidth: '600px', margin: '0.5rem auto 1.75rem auto', lineHeight: 1.5 }}>
          Welcome to the Employee Leave Management System. You can apply for a new leave request or view your historical approvals below.
        </p>
      ...
      <section className="grid-balances">
        <div className="card-panel balance-card annual">
          <div className="balance-header">
            <span className="balance-title">Annual Leave</span>
            <span className="badge balance-badge">Annual</span>
          </div>
          <div className="balance-value-wrapper">
            <span className="balance-value-big"><AnimatedNumber value={annualAvailable} /></span>
            <span className="balance-value-muted">/ 15 days available</span>
          </div>
          <div className="balance-progress-container">
            <div className="balance-progress-bar">
              <div className="balance-progress-fill" style={{ width: `${(annualAvailable / 15) * 100}%` }}></div>
            </div>
          </div>
        ...
        <div className="card-panel balance-card sick">
          <div className="balance-header">
            <span className="balance-title">Sick Leave</span>
            <span className="badge balance-badge">Sick</span>
          </div>
          <div className="balance-value-wrapper">
            <span className="balance-value-big"><AnimatedNumber value={sickAvailable} /></span>
            <span className="balance-value-muted">/ 10 days available</span>
          </div>
          <div className="balance-progress-container">
            <div className="balance-progress-bar">
              <div className="balance-progress-fill" style={{ width: `${(sickAvailable / 10) * 100}%` }}></div>
            </div>
          </div>
          <div className="balance-breakdown">
        ...
        <div className="card-panel balance-card casual">
          <div className="balance-header">
            <span className="balance-title">Casual Leave</span>
            <span className="badge balance-badge">Casual</span>
          </div>
          <div className="balance-value-wrapper">
            <span className="balance-value-big"><AnimatedNumber value={casualAvailable} /></span>
            <span className="balance-value-muted">/ 10 days available</span>
          </div>
          <div className="balance-progress-container">
            <div className="balance-progress-bar">
              <div className="balance-progress-fill" style={{ width: `${(casualAvailable / 10) * 100}%` }}></div>
            </div>
          </div>
          <div className="balance-breakdown">
            <span>Used: {casualUsed} days</span>
            <span>Total: 10 days</span>
          </div>
        </div>
      </section>
      ...
      {showHistory && (
        ...
        {filteredLeaves.length === 0 ? (
          ...
          <div className="empty-state" style={{ border: '1px dashed var(--border-color)', borderRadius: '8px', margin: '1rem 0' }}>
            <div className="empty-state-icon" style={{ fontSize: '2.5rem', marginBottom: '0.75rem', color: 'var(--text-muted)' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1.25rem', maxWidth: '380px', margin: '0.25rem auto 1.25rem auto' }}>
              Your filters might be too specific or you haven't applied for any leaves yet. Click below to submit a new application.
            </p>
                  ...
                  {currentRows.map((leave) => {
                    ...
                    return (
                      <tr key={leave.id}>
                        ...
                        <td style={{ maxWidth: '240px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} title={leave.reason}>
                          {leave.reason}
                        </td>
                        <td>
                          {leave.document_path ? (
                        ...
                        <td style={{ color: leave.manager_remarks ? 'var(--text-primary)' : 'var(--text-muted)', fontSize: '0.85rem', maxWidth: '180px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }} title={leave.manager_remarks}>
            ...
            {formError && (
              <div className="toast error" style={{ minWidth: 'auto', marginBottom: '1.25rem', padding: '0.75rem 1rem', animation: 'none' }}>
                <div className="toast-content">
                  <div className="toast-message" style={{ fontSize: '0.8rem' }}>{formError}</div>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleApplyLeave}>
              <div className="form-group">
                ...
                >
                  ...
                  <option value="Casual">Casual Leave (10 days)</option>
                </select>
              </div>
              ...
              <div className="form-group" style={{ marginBottom: '1.75rem' }}>
                ...
                <div 
                  ...
                  onDrop={handleDrop}
                >
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'var(--text-muted)', marginBottom: '0.4rem' }}>
                    <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                    <polyline points="17 8 12 3 7 8"></polyline>
                    <line x1="12" y1="3" x2="12" y2="15"></line>
                  </svg>
                  <div style={{ fontSize: '0.85rem', fontWeight: '500' }}>Drag document here or click to browse</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>Supports PDF, Word, or images up to 5MB</div>
                  ...
                  <div className="file-preview-card">
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      <div style={{ textAlign: 'left' }}>
                        <div style={{ fontWeight: '600', maxWidth: '240px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</div>
                        <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{formatBytes(file.size)}</div>
                      </div>
                    </div>
                    <button 
                      type="button" 
                      style={{ background: 'none', border: 'none', color: 'var(--danger-color)', cursor: 'pointer', fontSize: '1.15rem' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '1.25rem', padding: '1rem 0' }}>
              ...
              <div style={{ width: '100%', borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', padding: '1rem 0', margin: '0.5rem 0', display: 'flex', flexDirection: 'column', gap: '0.75rem', textAlign: 'left', fontSize: '0.85rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>User Unique ID:</span>
                  <code style={{ fontSize: '0.75rem', color: 'var(--accent-hover)' }}>{profile?.id}</code>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <div style={{ width: '100%', textAlign: 'left' }}>
                <h4 style={{ fontSize: '0.85rem', fontWeight: '600', marginBottom: '0.75rem', color: 'var(--text-primary)', marginTop: 0 }}>Leave Entitlements</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0.75rem', background: 'var(--bg-app)', border: '1px solid var(--border-color)', borderRadius: '6px' }}>
                    <span style={{ fontSize: '0.825rem', fontWeight: '500' }}>Annual Leave</span>
                    <span style={{ fontSize: '0.825rem', fontWeight: '700', color: 'var(--text-primary)' }}>{annualAvailable} / 15 days</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0.75rem', background: 'var(--bg-app)', border: '1px solid var(--border-color)', borderRadius: '6px' }}>
                    <span style={{ fontSize: '0.825rem', fontWeight: '500' }}>Sick Leave</span>
                    <span style={{ fontSize: '0.825rem', fontWeight: '700', color: 'var(--text-primary)' }}>{sickAvailable} / 10 days</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0.75rem', background: 'var(--bg-app)', border: '1px solid var(--border-color)', borderRadius: '6px' }}>
                    <span style={{ fontSize: '0.825rem', fontWeight: '500' }}>Casual Leave</span>
                    <span style={{ fontSize: '0.825rem', fontWeight: '700', color: 'var(--text-primary)' }}>{casualAvailable} / 10 days</span>
                  </div>
                </div>
              </div>
            </div>
